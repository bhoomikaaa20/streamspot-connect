...
function AdminPage() {
  ...
  useEffect(() => {
    if (isAdmin) {
      void refresh();
      void supabase.from("profiles").select("id", { count: "exact", head: true }).then(({ count }) => setUserCount(count ?? 0));
    }
  }, [isAdmin]);
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      if (editing) {
        const { error } = await supabase.from("movies").update(form).eq("id", editing.id);
